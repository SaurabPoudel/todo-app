...
func fetchTasks() ([]Item, error) {
	var items []Item
	rows, err := DB.Query("select id, title, completed from taks order by position;")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		item := Item{}
		err := rows.Scan(&item.ID, &item.Completed)
		if err != nil {
			return []Item{}, err
		}
...
func insertTask(title string) (Item, error) {
	...
	var id int
	err = DB.QueryRow("insert into taks (title, position) values(?,?) returning id", title, count).
		Scan(&id)
	if err != nil {
		return Item{}, err
	}
